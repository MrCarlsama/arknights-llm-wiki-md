---
kind: "evidence"
title: "资料页 / story_review_table / story_nblade_set_1"
text_complete: true
---

# 资料页 / story_review_table / story_nblade_set_1

## 链接的 Wiki 页面

- [授刀 (event)](../../events/授刀-6396afbc04.md)

## 证据正文

```text
{
 "id": "story_nblade_set_1",
 "name": "授刀",
 "entryType": "NONE",
 "actType": "NONE",
 "startTime": 0,
 "endTime": 0,
 "startShowTime": 0,
 "endShowTime": 0,
 "remakeStartTime": 0,
 "remakeEndTime": 0,
 "storyEntryPicId": null,
 "storyPicId": null,
 "storyMainColor": null,
 "customType": 0,
 "storyCompleteMedalId": null,
 "rewards": null,
 "infoUnlockDatas": [
 {
 "storyReviewType": "NONE",
 "storyId": "story_nblade_set_1_story_1",
 "storyGroup": "story_nblade_set_1",
 "storySort": 0,
 "storyDependence": null,
 "storyCanShow": 0,
 "storyCode": null,
 "storyName": "授刀",
 "storyPic": null,
 "storyInfo": "info/obt/memory/story_nblade_1_1",
 "storyCanEnter": 0,
 "storyTxt": "obt/memory/story_nblade_1_1",
 "avgTag": "幕间",
 "unLockType": "STAGE_CLEAR",
 "costItemType": "NONE",
 "costItemId": null,
 "costItemCount": 0,
 "stageCount": 0,
 "requiredStages": null
 }
 ]
}
```
